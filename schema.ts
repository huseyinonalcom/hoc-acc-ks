...
export const lists: Lists = {
  ...
  Material: list({
    ...
    fields: {
      ...
      width: float({ validation: { min: 0.1 } }),
      height: float({ validation: { min: 0.1 } }),
      length: float({ validation: { min: 0.1 } }),
      weight: float({ validation: { min: 0.1 } }),
      extraFields: json(),
    },
  }),
  Storage: list({
  ...
  StockMovement: list({
    ...
    fields: {
      ...
      stockAcc: float({ validation: { isRequired: true }, defaultValue: 0 }),
